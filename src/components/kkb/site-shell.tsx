  ...
  return (
    <header className="sticky top-0 z-40 border-b bg-background/95 backdrop-blur">
      ...
      <nav className="grid grid-cols-4 gap-2 px-4 pb-3 md:hidden" aria-label="Navigasi mobile">
        {navItems.map((item) => (
          <Link key={item.to} to={item.to} className="rounded-md border bg-card px-2 py-2 text-center text-xs font-black shadow-neo-sm" activeProps={{ className: "bg-brand-purple" }}>
            {item.label}
          </Link>
        ))}
      </nav>
    ...
    <footer className="border-t bg-foreground text-primary-foreground">
      <div className="mx-auto grid max-w-7xl gap-6 px-4 py-10 sm:px-6 md:grid-cols-[1.4fr_1fr_1fr] lg:px-8">
        <div className="rounded-lg border border-primary-foreground bg-brand-yellow p-6 text-foreground shadow-neo">
          ...
          <p className="mt-2 max-w-md font-bold">Solusi Gerobak Usaha Modern & Siap Pakai untuk brand kuliner, minuman, dan retail.</p>
        </div>
        <div>
          <p className="font-black">Kontak</p>
          <p className="mt-3 text-sm font-bold opacity-80">Jl. Workshop Kreatif No. 18, Indonesia</p>
